...
export const MediaCard: React.FC<MediaCardProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className={'group relative aspect-square bg-[#1A1D28] rounded-xl overflow-hidden cursor-pointer border transition-all duration-200 ' + borderClass}
    >
      <img
        src={thumbUrl}
        alt={media.filename}
      ...
      <div className='absolute top-2 left-2 right-2 flex items-center justify-between pointer-events-none'>
        <div className='flex items-center gap-1.5'>
          {isVideo && (
            <span className='px-2 py-0.5 rounded-md text-[11px] font-semibold bg-black/70 backdrop-blur-md text-white flex items-center gap-1'>
              <Play className='w-3 h-3 fill-white' />
              {formatDuration(media.duration_ms)}
            </span>
          )}
          {isLivePhoto && (
            <span className='px-2 py-0.5 rounded-md text-[11px] font-semibold bg-black/70 backdrop-blur-md text-[#2E7CF6] flex items-center gap-1'>
      ...
      <div className='absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/85 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end'>
        <p className='text-xs font-medium text-white truncate'>{media.filename}</p>
      </div>
    </div>
  );
};
